...
void GenerateAARE()
{
    ...
    uint8_t appContextName[]        = {0x60, 0x85, 0x74, 0x05, 0x08, 0x01, 0x03};
    uint8_t respondingAPTitle[]     = {0x4D, 0x4D, 0x4D, 0x00, 0x00, 0xBC, 0x61, 0x4E};
    uint8_t initiateRequestAPDU[]   = {0x28, 0x1F, 0x30, 0x01, 0x23, 0x45, 0x67, 0x89, 0x12, 0x14, 0xA0, 0x84, 0x5E, 0x47, 0x57, 0x14, 0x38, 0x3F, 0x65, 0xBC, 0x19, 0x74, 0x5C, 0xA2, 0x35, 0x90, 0x65, 0x25, 0xE4, 0xF3, 0xE1, 0xC8, 0x93};
    dlmsFormAARE( &stream, appContextName,sizeof(appContextName), respondingAPTitle,sizeof(respondingAPTitle), initiateRequestAPDU,sizeof(initiateRequestAPDU) );

    //printPDU( &data[0], (uint8_t*)stream );

    uint8_t expected[]  = {0x61, 0x48, 0xA1, 0x09, 0x06, 0x07, 0x60, 0x85, 0x74, 0x05, 0x08, 0x01, 0x03, 0xA2, 0x03, 0x02, 0x01, 0x00, 0xA3, 0x05, 0xA1, 0x03, 0x02, 0x01, 0x00, 0xA4, 0x0A, 0x04, 0x08, 0x4D, 0x4D, 0x4D, 0x00, 0x00, 0xBC, 0x61, 0x4E, 0xBE, 0x23, 0x04, 0x21, 0x28, 0x1F, 0x30, 0x01, 0x23, 0x45, 0x67, 0x89, 0x12, 0x14, 0xA0, 0x84, 0x5E, 0x47, 0x57, 0x14, 0x38, 0x3F, 0x65, 0xBC, 0x19, 0x74, 0x5C, 0xA2, 0x35, 0x90, 0x65, 0x25, 0xE4, 0xF3, 0xE1, 0xC8, 0x93};
    CU_ASSERT( memcmp(&data[0], &expected[0], sizeof(expected) ) == 0 );
}
